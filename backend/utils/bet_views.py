...
REJECT_LABELS = {
    ...
    'MATCH_NOT_FOUND': 'Матч не найден в базе',
...
def build_bet_view(bet: dict[str, Any], intent: dict[str, Any] | None = None, match: dict[str, Any] | None = None, match_cache: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
    intent = intent or {}
    bet = bet or {}
...
def search_blob(view: dict[str, Any]) -> str:
    pieces = [view.get('tx_id', ''), view.get('intent_hash', ''), view.get('sender_wallet', ''), view.get('match_id', ''), view.get('match_label', ''), view.get('league', ''), view.get('sport', ''), view.get('outcome_label', ''), view.get('status_label', ''), view.get('reject_label', '')]
